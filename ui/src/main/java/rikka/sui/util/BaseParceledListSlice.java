...
abstract class BaseParceledListSlice<T> implements Parcelable {
    ...
    static {
        ...
        try {
            ...
            Field field = IBinder.class.getDeclaredField("MAX_IPC_SIZE");
            field.setAccessible(true);
            size = (int) field.get(null);
        } catch (Throwable e) {
            Log.d(TAG, "failed to reflect MAX_IPC_SIZE", e);
        }
        if (size <= 0) {
            size = 64 * 1024;
        }
        ...
        Log.i(TAG, "MAX_IPC_SIZE is " + size);
    }
